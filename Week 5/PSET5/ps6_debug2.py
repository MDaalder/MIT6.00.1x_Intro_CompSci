...
#        bestShift = None
#        numWord = 0 # number of valid decrypted words for a given shift value
#        maxNumWord = 0 # best decryption
        
        Shift = list(range(0, 26)) # value of the shift
        bestShift = 0
        maxNumWord = 0
        
        #iterate over all valid shift values
        for n in Shift:
            
            # variable to keep track of number of valid decrypted words for a given shift value n
            numWord = 0
            # get the message with the shift applied, then turn into a list of separate words
            messageShifted = Message.apply_shift(self, n)
            listMessageShifted = messageShifted.split(' ')
            
            # iterate over the shifted words one by one
            for word in listMessageShifted:
                # if the shifted word is a real word, increase the variable numWord by 1
                if is_word(self.valid_words, word) == True:
                    numWord += 1
                
                # if the numWord for a given shift value is greater than for any previous shift values
                # then this shift value has lead to the most decrypted words.
                # store it for use in returning the final result.
                if numWord > maxNumWord:
                    maxNumWord = numWord
                    bestShift = n 
        
            # change the shift values
            PlaintextMessage.change_shift(self, n)
        
        
        # using the best shift value determined with the highest numValue,
        # decrypt the message, and return which shift value was used to get this result.
        decryptedText = Message.apply_shift(self, bestShift)
...
def decrypt_story():
    ...
    print('The encrypted text:', '\n',storytext.get_message_text(), '\n')
    return storytext.decrypt_message()
    


print('The shift number, and decrypted text!!!', '\n', decrypt_story())